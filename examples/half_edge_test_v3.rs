use rgeom::{self, HalfEdgeError, HalfEdgeResult, Vertex};
...
fn main(){
    ...
    let vertex = (|| -> Result<&Vertex, HalfEdgeError> {
    ...
    match vertex {
        Ok(vertex) => {
            // 成功時の処理
            println!("Vertex obtained: {:?}", vertex);
        }
        Err(e) => {
            // エラー時の処理
            eprintln!("Failed to obtain vertex: {}", e);
        }
    }
